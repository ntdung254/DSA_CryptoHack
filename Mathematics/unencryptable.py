from Crypto.Util.number import long_to_bytes, inverse, GCD

def solve_unencryptable():
    """ Duyệt qua các lũy thừa của 2 để tìm nhân tử p của N thông qua tính chất điểm cố định """
    N = 
    e = 65537
    c = 
    m = int("", 16)

    p = 1
    """ e - 1 = 2^16, ta bình phương m liên tiếp để tìm nhân tử """
    temp = m
    for _ in range(16):
        prev = temp
        temp = pow(temp, 2, N)
        if temp == 1:
            p = GCD(prev - 1, N)
            if 1 < p < N:
                break
    
    if p == 1:
        return

    q = N // p
    d = inverse(e, (p - 1) * (q - 1))
    print(long_to_bytes(pow(c, d, N)).decode())
  
solve_unencryptable()
